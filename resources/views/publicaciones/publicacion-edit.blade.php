            ...
            <div class="card d-lg-flex justify-content-lg-center" style="width: 80%;background: rgb(255,255,253);">
                <div class="card-body">
                    <form method="post" action="{{ route('publicacion.update', $publicacion->id) }}" enctype="multipart/form-data">
                        ...
                        <input class="form-control d-flex" type="text" style="width: 75%;" name="nombre" placeholder="Nombre" value="{{ $publicacion->nombre }}" required>
                        @error('nombre')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                        <label class="form-label" style="font-size: 24px;" for="descripcion">Descripción</label>
                        <textarea class="form-control" name="descripcion" placeholder="Descripcion" required>{{ $publicacion->descripcion }}</textarea>
                        @error('descripcion')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                        <div class="row">
                            <div class="col">
                                ...
                                <input class="form-control" type="number" style="width: 100%;" name="precio" step="any" placeholder="0.00" value="{{ $publicacion->precio }}" required>
                                @error('precio')
                                    <div class="alert alert-danger">{{ $message }}</div>
                                @enderror
                            </div>
